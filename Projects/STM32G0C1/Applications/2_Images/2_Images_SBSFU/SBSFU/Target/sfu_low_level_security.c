...

{
  ...
  /* Option Bytes access */
  {
    SFU_PROTECT_MPU_AREA_OB_BANK1_RGNV, SFU_PROTECT_MPU_AREA_OB_BANK1_START, SFU_PROTECT_MPU_AREA_OB_BANK1_SIZE,
    SFU_PROTECT_MPU_AREA_OB_BANK1_PERM, SFU_PROTECT_MPU_AREA_OB_BANK1_EXECV, SFU_PROTECT_MPU_AREA_OB_BANK1_SREG,
    SFU_PROTECT_MPU_AREA_OB_BANK1_TEXV, SFU_PROTECT_MPU_AREA_OB_BANK1_C, SFU_PROTECT_MPU_AREA_OB_BANK1_B
  },
                                                  ...
                                                  SFU_BoolTypeDef *pbIsProtectionToBeApplied)
{
  ...
  else
  {
    ...
    psFlashOptionBytes->USERType        = OB_USER_nBOOT_SEL;
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionIWDG(void)
{
  ...
  /* Set counter reload value to obtain 15 sec due to slow flash access and
  slow crypto operations. IWDG TimeOut.
  ...
  So Set Reload Counter Value = (15 * uwLsiFreq) / 256 */
...
__RAM_FUNC void SFU_LL_SECU_ActivateSecUser(uint32_t Address)
{
  ...
  } while (((* (uint32_t *)read_reg) & FLASH_CR_SEC_PROT) != FLASH_CR_SEC_PROT);

  if (((* (uint32_t *)read_reg) & FLASH_CR_SEC_PROT) != FLASH_CR_SEC_PROT)
  {
    /* Security issue : execution stopped ! */
    NVIC_SystemReset();
  }
